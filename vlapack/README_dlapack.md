...
| 既存 LAPACK + BLAS 部分のみ丸め保証 | 既存 liblapack + **dblas** (検証済み，README_rdblas.md 参照) |
...
`sandbox/tests/test_dlapack.cpp` が，4 つの丸めモード下で主要 wrapper の結果が
rdlapack の直接呼び出しと bit 一致すること，ipiv の 1-based 変換，
workspace query (LWORK = -1)，丸めモードの保存，および同一 binary 内での
MKL (`cblas_dgemm`) との共存を検証します:

```bash
g++ -I. -I${MKLROOT}/include -std=c++11 -DNDEBUG -DKV_FASTROUND -O3 -m64 \
-mavx512f -DVCP_USE_AVX512 \
sandbox/tests/test_dlapack.cpp \
-L${MKLROOT}/lib/intel64 -Wl,--no-as-needed \
-lmkl_intel_lp64 -lmkl_intel_thread -lmkl_core -liomp5 \
-lpthread -lm -ldl -lmpfr -fopenmp \
-o sandbox/bin/test_dlapack

./sandbox/bin/test_dlapack   # => test_dlapack: ALL PASS
```

## License

dlapack 自体は wrapper ですが，呼び出す rdlapack は reference LAPACK 3.12.1 の
派生物です．`vlapack/LICENSE_LAPACK.txt` を参照してください．
