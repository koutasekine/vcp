...
各 backend は cache size と thread 数のみから block size を導出します。
環境 (特定の CPU 名や行列形状) に特化した分岐は持ちません。

- 検出する情報
  - L1d / L2 / L3 cache size (`/sys/devices/system/cpu/cpu0/cache/`)
  - core あたりの hardware thread 数 (`topology/thread_siblings_list`)
- block size の決め方
