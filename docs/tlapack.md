...
| `tlapack_dd.hpp` | `T=kv::dd` の明示的特殊化．double LAPACK/BLAS を前処理として使い，`kv::dd` 残差反復で解を改善する |
...
## T=kv::dd の混合精度残差反復特殊化

`T=kv::dd` については `vcp/tlapack/tlapack_dd.hpp` を include すると，一部 routine が
明示的特殊化されます．この特殊化は，計算の初期解または補正方程式を
`double` LAPACK/BLAS で高速に解き，残差や更新を `kv::dd` で行う混合精度型の
実装です．

```cpp
#include <vcp/tlapack/tlapack_dd.hpp>

int info = vcp::tgesv<kv::dd>(n, nrhs, A, lda, ipiv, B, ldb);
int info2 = vcp::tsyev<kv::dd>('V', 'U', n, A, lda, w);
```
...
を include します．したがって `tlapack_dd.hpp` だけを include しても動作します．
一方，これらの高速特殊化 header が先に include されている場合も include guard により
同じ特殊化を使います．
...
// または，高速特殊化 header を先に include してからでもよい
...
ただし `tlapack_dd.hpp` は `t...<kv::dd>` を最初に使う前に include してください．
明示的特殊化は，対象 template が同じ翻訳単位で既に暗黙実体化された後には
追加できません．
...
この特殊化は高速化を目的とした近似計算です．補正方程式を `double` で解くため，
悪条件問題に対しては汎用 `t...<kv::dd>` 実装より適用範囲が狭くなります．

特に一般連立一次方程式では，反復改良が有効に働くには概ね

```text
cond(A) * eps_double < 1
```

に近い条件が必要です．`eps_double` は約 `2e-16` なので，`cond(A)` が
`1e15` 付近以上では残差反復が停滞しやすく，`double` の LU/Cholesky 分解が
数値的に破綻する場合があります．この場合，`kv::dd` の汎用実装なら解ける問題でも，
`tlapack_dd.hpp` の特殊化では失敗したり，`kv::dd` 精度まで改善できないことがあります．

固有値問題でも，近接固有値・重複固有値・固有ベクトルが悪条件な場合には，
