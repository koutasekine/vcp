...
rdlapack (LAPACK 層) は `vlapack/` に，依存する rdblas / dblas / rmatmul
(BLAS 層) は兄弟 directory の `vblas/` に置かれています:
...
**`vlapack/LICENSE_LAPACK.txt`** に収録しています (license の条件である
