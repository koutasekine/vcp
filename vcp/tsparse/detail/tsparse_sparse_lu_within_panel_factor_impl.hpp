...
// ---------------------------------------------------------------------------
// [SLU-CLN1 C1, 2026-07-05] run_within_panel_factorization (the transitional
// top-level SS17.2(B) prototype driver) was REMOVED together with the
// prototype pass in the factorize pipelines (results were discarded by an
// idempotent re-bootstrap).  The production SS17.2(B) entry point is
// factorize_within_panel_single in tsparse_sparse_lu_true_numeric_impl.hpp
// (interleaved per-supernode), which consumes the SHARED components above
// (workspace gather, pivot search, row swap, scale, trailing update,
// scatter).  Tests exercising SS17.2(B) semantics call
// factorize_within_panel_single directly (SLU-CLN1 D-4 migration).
// ---------------------------------------------------------------------------
